...
data_dir = os.path.join(os.path.dirname(__file__), "../Unique_Data/")
...
uniqueFiles = os.listdir(os.path.join(os.path.dirname(__file__), "../Unique_Data/"))
...
def selectFeatures(train_set, train_text, k):
    ...
    words = [word for word in tokens if word not in english_stops and len(
    ...
    words = [word for (word, tag) in tagged_words if tag in [
        'NN', 'JJ', 'NNS', 'RB', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']]
    ...
    for w in dictionary:
        ...
        if (N11 != 0) and (N1_ != 0) and (N_1 != 0):
            A_tc = A_tc + (float(N11)/float(N) *
                           math.log(float(N*N11)/float(N1_*N_1), 2))
        if (N01 != 0) and (N0_ != 0) and (N_1 != 0):
            A_tc = A_tc + (float(N01)/float(N) *
                           math.log(float(N*N01)/float(N0_*N_1), 2))
        if (N10 != 0) and (N1_ != 0) and (N_0 != 0):
            A_tc = A_tc + (float(N10)/float(N) *
                           math.log(float(N*N10)/float(N1_*N_0), 2))
        if (N00 != 0) and (N0_ != 0) and (N_0 != 0):
            A_tc = A_tc + (float(N00)/float(N) *
                           math.log(float(N*N00)/float(N0_*N_0), 2))
        # print w, A_tc
        # Append to the list
        l.append((A_tc, w))
...
def classify():
    ...
    for i in range(1, num_rows):
        number = (worksheet.cell_value(i, 0).strip())
        reason = (worksheet.cell_value(i, Reason_Index).strip())
        action = (worksheet.cell_value(i, Action_Index).strip())
        fault_class = str(worksheet.cell_value(
            i, Fault_Index).strip())
    ...
    for line in dic:
        ...
        print('word and score', word, score)
    ...
    print(len(features))
    [P_tc, P_tc_] = training(train_set, features[1:int(len(features)/2)])
