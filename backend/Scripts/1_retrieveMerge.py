...
def initProg(startDate, endDate):
    ...
    query = 'start_search=1&postdatefrom='+startDate + \
        '&postdateto='+endDate+'&PAGENUM=500&sortcolumn=cda'
    ...
    try:
        # get the text
        text = line.text.strip()#str(line.text, 'utf-8').strip()
    ...
    for link in mech.links():
        if (link.url.find('/res.cfm?id') > -1):
            ...
            td = soup.find(
                "td", style="font-family: arial; color: #23238e; font-weight:bold; font-size:11pt;")
            if("Class" in str(td)):
            ...
            for link in mech.links():
                ...
                if (link.url.find('/classification.cfm?ID=') > -1):
                    ...
                    soup2 = BeautifulSoup(mech.response().read(), features="html5lib")
                    table2 = soup2.find(
            ...
            table = [t for t in tables if t.find(
                text=re.compile('Recall Number'))]
            if(len(table) != 0):
                table = table[0]
            else:
                continue
            ...
            if ('Terminated' in varis[status_indx]):
                if('on ' in varis[status_indx]):
                    ...
                    varis[tdate_indx] = str(varis[status_indx]).split('on ')[
                        1].rstrip()
